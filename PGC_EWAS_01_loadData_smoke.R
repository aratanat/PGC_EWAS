...
setwd("/Users/ar3054/Dropbox/PTSD methylation workgroup Results Files/Smoking/")
homeDir<-"/Users/ar3054/Documents/R/PGC_EWAS/Current/PGC_EWAS/Smoking/"
...
load("DNHS100_limma_smoking_results_11.07.16.Rdata")
DNHS.coef<-fit.coef
rm(fit.coef)
...
load("GTP_limma_results_smoking_02.26.17.Rdata")
GTP.coef<-fit.coef
rm(fit.coef)
results<-results[order(results[, "P.Value"]), ]
GTP.results<-results
...
load("MRS_limma_smoking_results_jul7_2016.Rdata")
MRS.coef<-fit.coef
rm(fit.coef)
results<-results[order(results[, "P.Value"]), ]
rank<-c(1:nrow(results))
...
load("BostonVA_Jun_3_2016_limma_smoking_results.Rdata")
VA.coef<-fit.coef
rm(fit.coef)
results<-results[order(results[, "P.Value"]), ]
rank<-c(1:nrow(results))
...
load("Utrecht_limma_smoking_results.Rdata")
...
load("WTC180_meta_limma_smoking_results_08.09.16.Rdata")
WTC.coef<-fit.coef
rm(fit.coef)
results<-results[order(results[, "P.Value"]), ]
rank<-c(1:nrow(results))
...
load("Duke_limma_smoking_results_052416.Rdata")
DUKE.coef<-fit.coef
rm(fit.coef)
results<-results[order(results[, "P.Value"]), ]
rank<-c(1:nrow(results))
...
load("STARRS_limma_smoking_results_v1dich_aug22_2016.Rdata")
AS.coef<-fit.coef
rm(fit.coef)
results<-results[order(results[, "P.Value"]), ]
rank<-c(1:nrow(results))
...
load("Duke_AAset1_limma_smoking_results_052416.Rdata")
MIR.coef<-fit.coef
rm(fit.coef)
results<-results[order(results[, "P.Value"]), ]
rank<-c(1:nrow(results))
...
setwd(homeDir)
save.image("PGC_EWAS_combinedData_smoke.Rdata")
rm(list=ls())
...
write.csv(tab, "PGC_EWAS_individualCohort_results_smoking.csv")
